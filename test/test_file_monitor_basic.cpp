...
int test_file_monitor_basic(int argc, char* argv[])
{
    ...
    file_monitor.add(
        "test/t1.txt", [](const fs::path& _dir, const fs::path& _name, const chrono::system_clock::time_point& _time_point) {
            const std::time_t t_c = std::chrono::system_clock::to_time_t(_time_point);
            cout << "Modified: " << _dir << "/" << _name << " "
                 << std::put_time(std::localtime(&t_c), "%F %T") << endl;
        });
    this_thread::sleep_for(chrono::seconds(1));
    file_monitor.add(
        "test/t2.txt", [](const fs::path& _dir, const fs::path& _name, const chrono::system_clock::time_point& _time_point) {
            const std::time_t t_c = std::chrono::system_clock::to_time_t(_time_point);
            cout << "Modified: " << _dir << "/" << _name << " "
                 << std::put_time(std::localtime(&t_c), "%F %T") << endl;
        });
    this_thread::sleep_for(chrono::seconds(1));
    file_monitor.add(
        "test/t3.txt", [](const fs::path& _dir, const fs::path& _name, const chrono::system_clock::time_point& _time_point) {
            const std::time_t t_c = std::chrono::system_clock::to_time_t(_time_point);
            cout << "Modified: " << _dir << "/" << _name << " "
                 << std::put_time(std::localtime(&t_c), "%F %T") << endl;
        });
